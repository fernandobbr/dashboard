...
function openModalDetalhe(recurso, tipo) {
  ...
  var cols = tr2 ? ['OP PAI', 'Descrição', 'Recurso', 'Peso Pai', 'Situação', 'Preparação', 'Semana', 'Saldo Disp.'] : ['OP PAI', 'Descrição', 'Peso Pai', 'Situação', 'Preparação', 'Semana', 'Saldo Disp.'];
  ...
  document.getElementById('modalBody').innerHTML = rows.sort(function (a, b) { return String(a.op).localeCompare(String(b.op)); }).map(function (r) { var sc = !r.temNao ? '<span style="font-family:\'IBM Plex Mono\',monospace;font-size:11px;font-weight:700;padding:2px 8px;background:rgba(31,221,158,.1);color:var(--accent);border:1px solid rgba(31,221,158,.4)">SIM</span>' : '<span style="font-family:\'IBM Plex Mono\',monospace;font-size:11px;font-weight:700;padding:2px 8px;background:rgba(255,77,109,.1);color:var(--red);border:1px solid rgba(255,77,109,.4)">NÃO</span>'; var base = '<tr><td class="op-c">' + r.op + '</td><td style="font-size:11px;color:var(--text2)">' + (r.produto || '—') + '</td>'; if (tr2) base += '<td style="font-size:11px;color:var(--text2)">' + r.recurso + '</td>'; base += '<td class="mono" style="color:var(--cyan)">' + fmtKg(r.pesoPai) + '</td><td><span class="chip ' + chipCls(r.situacao) + '">' + (r.situacao || '—') + '</span></td><td><span class="chip ' + prepCls(r.prep) + '">' + (r.prep || 'Não Iniciada') + '</span></td><td class="mono" style="color:var(--text2);font-size:11px">' + (r.semana ? 'Sem ' + r.semana : '—') + '</td><td>' + sc + '</td></tr>'; return base; }).join('');
